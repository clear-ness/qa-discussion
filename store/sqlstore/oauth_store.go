...
func NewSqlOAuthStore(sqlStore store.Store) store.OAuthStore {
	s := &SqlOAuthStore{
		Store: sqlStore,
	}

	for _, db := range sqlStore.GetAllConns() {
		db.AddTableWithName(model.OAuthApp{}, "OAuthApps").SetKeys(false, "Id")

		db.AddTableWithName(model.AuthData{}, "OAuthAuthData").SetKeys(false, "Code")
		db.AddTableWithName(model.AccessData{}, "OAuthAccessData").SetKeys(false, "Token")
	}
...
func (as SqlOAuthStore) GetAppByUserId(userId string, offset, limit int) ([]*model.OAuthApp, *model.AppError) {
	var apps []*model.OAuthApp

	if _, err := as.GetReplica().Select(&apps, "SELECT * FROM OAuthApps WHERE UserId = :UserId LIMIT :Limit OFFSET :Offset", map[string]interface{}{"UserId": userId, "Offset": offset, "Limit": limit}); err != nil {
		return nil, model.NewAppError("SqlOAuthStore.GetAppByUserId", "store.sql_oauth.get_app_by_user_id.find.app_error", nil, "user_id="+userId+", "+err.Error(), http.StatusInternalServerError)
	}
...
func (as SqlOAuthStore) deleteAppExtras(transaction *gorp.Transaction, clientId string) *model.AppError {
	if _, err := transaction.Exec(
		`DELETE FROM
			OAuthAuthorizedApps
		WHERE
			ClientId = :ClientId`, map[string]interface{}{"ClientId": clientId}); err != nil {
		return model.NewAppError("SqlOAuthStore.DeleteApp", "store.sql_oauth_authorized.delete.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
...
func (as SqlOAuthStore) GetAuthorizedApps(userId string, offset, limit int) ([]*model.OAuthApp, *model.AppError) {
	...
	if _, err := as.GetReplica().Select(&apps,
		`SELECT o.* FROM OAuthApps AS o INNER JOIN
			OAuthAuthorizedApps AS a ON a.ClientId=o.Id AND a.UserId=:UserId LIMIT :Limit OFFSET :Offset`, map[string]interface{}{"UserId": userId, "Offset": offset, "Limit": limit}); err != nil {
...
func (as SqlOAuthStore) DeleteAuthorizedApp(userId string, clientId string) *model.AppError {
	if _, err := as.GetMaster().Exec(
		`DELETE FROM
			OAuthAuthorizedApps
		WHERE
			UserId = :UserId
			AND ClientId = :ClientId`, map[string]interface{}{"UserId": userId, "ClientId": clientId}); err != nil {
		return model.NewAppError("SqlOAuthStore.DeleteAuthorizedApp", "store.sql_oauth_authorized.delete.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
...
func (as SqlOAuthStore) SaveAuthorizedApp(app *model.OAuthAuthorizedApp) *model.AppError {
	if err := app.IsValid(); err != nil {
		return err
	}

	if err := as.GetMaster().Insert(app); err != nil {
		return model.NewAppError("SqlOAuthStore.SaveAuthorizedApp", "store.sql_oauth.save_authorized_app.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
